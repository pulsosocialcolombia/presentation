\documentclass[11pt,oneside,a4paper,english,french,spanish,brazil]{abntex2}

% ----------------------------------------------------------------------------------------
% - Librerias
  \usepackage{sty/modal_configs}    
  \usepackage{sty/datos_portada}      
  \usepackage{sty/modal_dados}              
   \usepackage{sty/modal_vocabulario}         
  \usepackage{sty/modal_leiaute_rt} 
  
  \definecolor{mycolor}{RGB}{178,74,59}% Rule colour
  
\captionsetup[figure]{labelformat=empty}% redefines the caption setup of the figures environment in the beamer class.


% ----------------------------------------------------------------------------------------
% - INICIO
  \begin{document}
%      \begin{sloppypar}

          % Seleciona o idioma do documento (conforme pacotes do babel)
            \selectlanguage{spanish}

          % Retira espaço extra obsoleto entre as frases.
            \frenchspacing

          % ------------------------------------------------------------------------------
          % - ESTRUTURA DO DOCUMENTO
          % ------------------------------------------------------------------------------
          % ------------------------------------------------------------------------------
          % - PRÉ-TEXTUAL
          % ------------------------------------------------------------------------------
          \pagestyle{modalpre}

          % Portada
            \input{sty/00_portada}
            \clearpage

          % Metadata
            \input{sty/01_autores}
            \clearpage

          % Sumarios
            \vspace{1cm}
            \tableofcontents
            \clearpage
            
            \vspace{1cm}
            \listoffigures
            \clearpage

            \vspace{1cm}
            \listoftables

            \clearpage
  
          % ------------------------------------------------------------------------------
          % - Introducción
          % -----------------------------------------------------------------------------
          \input{sty/02_Introduccion}
          \clearpage
          \newpage
          
          \pagestyle{modal}
          \input{reporte_partes/00_Introduccion}
           \clearpage
        % ------------------------------------------------------------------------------
          % - Contexto
          % -----------------------------------------------------------------------------
          \input{sty/03_Contexto}
          \clearpage
          \newpage
          
          \pagestyle{modal}
 
           \input{reporte_partes/02_Contexto}
           \clearpage
           
            % ------------------------------------------------------------------------------
          % - Resultados
          % -----------------------------------------------------------------------------
          \input{sty/04_Resultado}
          \clearpage
          \newpage
        
          
          \pagestyle{modal}
 
           \input{reporte_partes/03_Resultado}
           \clearpage
           
        % ------------------------------------------------------------------------------
          % - Resultados
          % -----------------------------------------------------------------------------
          \input{sty/05_Colombias}
          \clearpage
          \newpage


           \input{reporte_partes/04_Colombias}
           \clearpage
           
           
            % ------------------------------------------------------------------------------
          % - Conclusiones
          % -----------------------------------------------------------------------------
          \input{sty/06_Conclusiones}
          \clearpage
          
          \pagestyle{modal}
           \input{reporte_partes/05_Conclusiones}
           \clearpage

          % ------------------------------------------------------------------------------
          % - PÓS-TEXTUAL
          % ------------------------------------------------------------------------------
          \pagestyle{modalpre}

          % Referências bibliográficas
            \clearpage
            \bibliography{references.bib}

          % Glossário
            %\glossary              % Consulte o manual da classe abntex2 para orientações sobre o glossário.
            
            
             % Anexos
             \part*{Anexos}
             \begin{anexosenv}
             
             \chapter{Protocolo de Manejo de Datos - Pulso Social}\label{anex:anexo1}
                \input{anexos/A1_protocolo_datos}
             
             
            \chapter{Análisis de Componentes Principales por Dimensiones}\label{anex:anexo1}
                \input{anexos/A2_Anexo_PCA}
                
            
            \chapter{Análisis de Clústers}\label{anex:anexo1}
                \input{anexos/A3_Anexo_Cluster}
                
 
             \end{anexosenv}


          % Índice remissimo
            \printindex



%    \end{sloppypar}
\end{document}



                

\end{document}


         
            